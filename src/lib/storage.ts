...
export const saveToken = (id: string, token: string, type: 'booking' | 'comment'): void => {
...
export const verifyToken = (id: string, token: string, type: 'booking' | 'comment'): boolean => {
  const existingTokensStr = localStorage.getItem(TOKENS_KEY) || '{}';
  const existingTokens = JSON.parse(existingTokensStr);
...
export const updateCommentStatus = (bookingId: string, commentId: string, status: 'draft' | 'published'): void => {
  const bookings = getBookings();
  const booking = bookings.find(b => b.id === bookingId);
